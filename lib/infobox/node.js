define(['sorttable', 'snabbdom', 'd3-interpolate', 'moment', 'helper'],
  function (SortTable, V, d3Interpolate, moment, helper) {
    ...
    function showStatus(d) {
      return function (el) {
        el.classList.add(d.unseen ? 'unseen' : (d.online ? 'online' : 'offline'));
        el.textContent = _.t((d.online ? 'node.lastOnline' : 'node.lastOffline'), {
    ...
    function showClients(d) {
      ...
      return function (el) {
        var text =  _.t('none');
        if (d.statistics.clients.total > 0) {
          text = d.statistics.clients.total + ' ' + _.t('node.clients') + ' (';
          var lanclient = d.statistics.clients.total - d.statistics.clients.wifi;
          if (d.statistics.clients.wifi24 > 0) {
            text += d.statistics.clients.wifi24 + 'x 2.4 Ghz, ';
          }
          if (d.statistics.clients.wifi5 > 0) {
            text += d.statistics.clients.wifi5 + 'x 5 Ghz, ';
          }
          if (lanclient > 0) {
            text += ' ' + lanclient + 'x LAN, ';
          }
          if (text.slice(-1) !== '(' ) {
            text = text.slice(0, -2) + ')';
          } else {
    ...
    function showAirtime(d) {
      if (!('wireless' in d.statistics)) {
        return undefined;
      }

      return function (el) {
        d.statistics.wireless.forEach(function (item) {
          var width = item.ChanUtil / 100;
          var value = ( ( item.frequency < 5000 ) ? '11g' : '11a' ) + ': ' + Math.round(item.ChanUtil) + ' %';
          var warning = false;
          if (width > 0.9) {
            warning = true;
          }
    ...
    function showRAM(d) {
      if (!('memory' in d.statistics)) {
        return undefined;
      }

      return function (el) {
        var width = 1 - (d.statistics.memory.free + d.statistics.memory.buffers + d.statistics.memory.cached) / d.statistics.memory.total;
        var value = Math.round(width * 100) + ' %';
        var warning = false;
        if (width >= 0.8) {
          warning = true;
        }
    ...
    return function (config, el, router, d, gateways) {
      ...
      function renderNeighbourRow(n) {
        ...
        var td2 = V.h('td', name);
        var td3 = V.h('td', n.node.statistics.clients.total ? n.node.statistics.clients.total.toString() : '0');
      ...
      helper.attributeEntry(attributes, 'node.gateway', d.nodeinfo.vpn ? 'ja' : null);
      ...
      if (config.nodeInfobox && config.nodeInfobox.hardwareUsage) {
        ...
        helper.attributeEntry(attributes, 'node.airtime', showAirtime(d));
      }
      ...
      if (d.neighbours.length > 0) {
        ...
        var headings = [{
          ...
          class: 'ion-people',
          sort: function (a, b) {
            return ('clients' in a.node.statistics ? a.node.statistics.clients.total : -1) -
              ('clients' in b.node.statistics ? b.node.statistics.clients.total : -1);
          },
          reverse: true
        }, {
          name: 'node.tq',
