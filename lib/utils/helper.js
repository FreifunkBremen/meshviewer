...
define({
  ...
  getJSON: function getJSON(url) {
    return require('helper').get(url).then(function (res) {
      var result = JSON.parse(res.response);
      result._lastModify = res.getResponseHeader('Last-Modified');
      return result;
    });
  },
  ...
  positionClients: function positionClients(ctx, p, startAngle, clients, startDistance) {
    var lanclient = clients.total - clients.wifi;
    var wifi5 = clients.wifi5;
    ...
    for (var orbit = 0, i = 0; i < clients; orbit++) {
      ...
      for (var j = 0; j < Math.min(delta, n); i++, j++) {
        if (i >= (clients - wifi5 - lanclient) && ctx.fillStyle !== 'rgba(147, 0, 220, 0.7)') {
          ctx.fill();
          ctx.beginPath();
          ctx.fillStyle = 'rgba(147, 0, 220, 0.7)';
        }
        if (i >= (clients - lanclient) && ctx.fillStyle !== 'rgba(220, 128, 0, 0.7)') {
          ctx.fill();
          ctx.beginPath();
          ctx.fillStyle = 'rgba(220, 128, 0, 0.7)';
        }
  ...
  mapToArray: function mapToArray(map) {
    return Object.keys(map).map(function (i) {
      return map[i];
    });
  }
});
